 ...
 *
 */
var whichPackageManager = async function whichPackageManager() {
    var res = await whichpm(process.cwd());
    var name = res.name;
    return name;
};
...
var getInstallCommand = async function getInstallCommand(name, dev) {
    var packageManager = await whichPackageManager();
...
var installModule = async function installModule(_ref2) {
    var name = _ref2.name,
        dev = _ref2.dev;

    var spinner = startSpinner('Installing ' + name, 'green');

    var command = await getInstallCommand(name, dev);
