...
let getUsedModules = () => {
  ...
  for (let fileName of files) {
    ...
    for (let name of modulesFromFile) usedModules.push({
      name,
      dev
    });
  }
...
/* Enable and show notifications */


let showNotification = message => {
  notifier.notify({
    title: "auto-install running",
    message,
    open: void 0,
    wait: false
  });
...
let isModulePopular = (name, callback) => {
  let spinner = startSpinner("Checking ".concat(name), "yellow");
  let url = "https://api.npmjs.org/downloads/point/last-month/".concat(name);
  https.get(url).then(response => {
    ...
    let message = "Could not connect to npm, check your internet connection!";
...
let installModule = async (_ref2, notifyMode) => {
  let {
    name,
    dev
  } = _ref2;
  let spinner = startSpinner("Installing ".concat(name), "green");
  let command = await getInstallCommand(name, dev);
  let message = "".concat(name, " installed");
  ...
  if (success) {
    ...
    stopSpinner(spinner, "".concat(name, " installation failed"), "yellow", notifyMode);
  }
};
/* is scoped module? */
...
let installModuleIfTrustedAuthor = (_ref3, notifyMode) => {
  let {
    name,
    dev
  } = _ref3;
  ...
  packageJson(name).then(json => {
    if (json.author && json.author.name === trustedAuthor) {
      ...
      console.log(colors.red("".concat(name, " not trusted")));
      if (notifyMode) showNotification("".concat(name, " not trusted"));
    }
  });
};
...
let installModuleIfTrusted = (_ref4, notifyMode) => {
  let {
    name,
    dev
  } = _ref4;
...
let uninstallModule = (_ref5, notifyMode) => {
  let {
    name,
    dev
  } = _ref5;
  ...
  let message = "".concat(name, " removed");
  let spinner = startSpinner("Uninstalling ".concat(name), "red");
  runCommand(command);
