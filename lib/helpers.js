...
var installModule = function installModule(_ref2) {
    ...
    var command = 'npm install ' + name + ' --save';
...
var installModuleIfTrusted = function installModuleIfTrusted(_ref3) {
    var name = _ref3.name;
    var dev = _ref3.dev;

    isModulePopular(name, function (popular) {
        if (popular) installModule({ name: name, dev: dev });else console.log(colors.red(name + ' not trusted'));
    });
};

/* Uninstall module */

var uninstallModule = function uninstallModule(_ref4) {
    var name = _ref4.name;
    var dev = _ref4.dev;

    if (dev) return;

    var command = 'npm uninstall ' + name + ' --save';
...
var removeFilePaths = function removeFilePaths(modules) {
    ...
    try {
        for (var _iterator6 = modules[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
            ...
            _module2.name = _module2.name.split('/')[0];
        }
    } catch (err) {
