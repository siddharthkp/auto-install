...
let installModuleIfTrustedAuthor = ({ name, dev }) => {
    ...
    packageJson(name).then(json => {
        if (json.author && json.author.name === trustedAuthor) installModule({ name, dev });else console.log(colors.red(`${ name } not trusted`));
    });
};
...
let installModuleIfTrusted = ({ name, dev }) => {
    ...
    if (isScopedModule(name)) installModule({ name, dev });else {
